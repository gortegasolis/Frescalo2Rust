...
/// This is the whole FRESCALO analysis, run start to finish, in the same interactive
/// prompt-by-prompt order as the original Fortran program. It asks for: the occurrence records
/// (site, species, time period), the neighbourhood weights produced by `neighsim`, an optional
/// list of species to exclude from the benchmark set, three output file names, a target local
/// frequency Φ, and a benchmark limit R*.
///
/// For every site it then: gathers presence/absence records from that site's whole neighbourhood
/// into a weighted local frequency for each species; rescales those frequencies so that recording
/// effort becomes comparable across sites, however unevenly they were actually surveyed (delegated
/// to `fresca`); and finally estimates, for every species and time period, a time factor
/// describing how that species' frequency has changed (delegated to `tfcalc`), writing the results
/// to the trends file.
///
/// The Φ and R* it prompts for (defaulting to 0.74 and 0.2703) are the same two parameters the
/// method is built around: Φ is the standard frequency a well-sampled neighbourhood is assumed to
/// reach, and R* sets the cutoff rank below which a species counts as a "benchmark" species for
/// measuring recording effort at that site (Hill 2011, p.199–200). The 0.74 default matches the
/// value Hill used for the bryophyte data set discussed in the paper, chosen from the high end of
/// observed neighbourhood frequencies because bryophytes are never quite exhaustively recorded
/// (Hill 2011, p.199).
...
/// A one-line helper: writes a string followed by a newline to a file. It's plumbing, not part of
/// the method itself — used throughout `frescalo` to write the log file and the headers of the
/// output tables.
...
/// These two write one line of the trends output file per species/time-period combination: the
/// time factor and its standard deviation computed by `tfcalc`, plus supporting counts (how many
/// records, how many sites had non-zero/near-saturated detection probability). `write_trend`
/// writes an actual computed result; `write_zero_trend` writes a padding row (all zeros) for
/// species/period combinations that had no records at all, so that every species has one row per
/// time period in the output even where it was never seen — this keeps the trend series unbroken
/// for plotting change over time, as in the paper's epiphyte example (Hill 2011, Fig. 6, p.201).
...
// ---------------------------------------------------------------------------
/// These two functions implement the two central calculations of the FRESCALO method (Hill 2011):
/// standardizing recording effort at a site (`fresca`) and turning standardized frequencies into a
/// per-species trend over time (`tfcalc`). Everything else in `frescalo` is really scaffolding
/// around these two functions.
///
/// For one site, this takes the raw local frequency of every species (how often it turns up across
/// that site's neighbourhood) and finds a single multiplier, α, that rescales those frequencies so
/// the neighbourhood's frequency-weighted mean frequency Φ matches a fixed target — the "sampling
/// effort multiplier" of the method (Hill 2011, p.196, p.199). Intuitively: a well-recorded
/// neighbourhood needs little inflation (α close to 1), while a poorly-recorded one needs its raw
/// frequencies inflated a lot (larger α) to reach the same standard. α is found by repeatedly
/// re-scaling a trial value until the rescaled mean frequency converges on the target Φ, mirroring
/// the successive-approximation procedure described in the paper (Hill 2011, p.199). 1/α is later
/// reported as that site's estimate of recording effort. Once α converges, this also writes one
/// line to the samples-statistics file and one line per species to the rescaled-frequencies file —
/// the two main per-site/per-species output tables of the analysis.
#[allow(clippy::too_many_arguments)]
...
// ---------------------------------------------------------------------------
/// For one species in one time period, this estimates a **time factor**: a single number
/// describing how that species' true frequency in that period compares to a baseline of 1, given
/// how much sampling effort was actually spent looking for it at each site and how detectable it
/// was. This is the xjt of the paper's trend model, found the same way the paper describes: by
/// starting from a trial value of 1 and repeatedly rescaling it until the predicted number of
/// records matches the observed number (Hill 2011, p.200). A time factor above 1 means the species
/// was found more than expected for that level of effort (increasing); below 1 means less than
/// expected (declining). It also returns a standard error for the time factor, obtained by
/// re-running the same iteration against a target one standard deviation higher than the observed
/// total (Hill 2011, p.200).
