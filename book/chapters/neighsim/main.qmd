...
Part of `src/bin/neighsim.rs` (`neighsim` binary/module).

<!-- TODO: describe what this function does, and, where relevant, which Fortran subroutine/statement it ports. -->

## What this does

`neighsim` builds the neighbourhood weights that `frescalo` actually uses —
this is the direct implementation of stage 1 of the method, which defines
each site's neighbourhood as a weighted set of nearby sites, weighted by
both spatial proximity and floristic (species-composition) similarity
(Hill 2011, p.196).

It takes two inputs: the [`sampdist`](../sampdist/main.qmd) output (each
site's nearest neighbours by physical distance) and a training-set species
list per site (typically a well-recorded reference taxon group, used only
to judge floristic similarity, not the species being analysed). For every
pair of sites it computes Sørensen's similarity coefficient — twice the
number of shared training-set species divided by the sum of each site's
species total — then combines that floristic-similarity rank with the
spatial-proximity rank from `sampdist` using the same smooth, bounded
weighting curve as the paper: each rank is scaled to 0–1 and passed through
`(1 - rank²)⁴`, so the target site itself gets full weight 1, and weight
falls off smoothly (not abruptly) the further down each ranking a
neighbour is, reaching zero beyond the neighbourhood size chosen (Hill
2011, p.197). The final weight for a neighbour is the product of its
spatial and floristic weights — the `w_ii'` that `frescalo` reads in and
uses to pool records into local frequencies.

```{.rust filename="src/bin/neighsim.rs" tangle="neighsim__main"}
fn main() {
